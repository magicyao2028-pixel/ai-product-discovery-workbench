...
Each proposed record retains the source note ID, excerpt IDs, claim ID, observation date, synthetic flag, and `indicative` reliability. It is labelled `proposed_not_merged` and does not enter the opportunity score, requirement gate, feedback decision log, or current PRD.
